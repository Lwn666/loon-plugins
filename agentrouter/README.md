...
站点同时下发**两个** cookie，且顺序不固定：

| Cookie | 来源 | 出现时机 |
|---|---|---|
| `acw_tc` | 阿里云 WAF 流量标记 | **每次请求都有**（错误密码也下发） |
| `session` | 真正的登录凭证 | 仅登录成功时下发 |

**只取第一个 Set-Cookie 会时好时坏** —— 拿到 `acw_tc` 就 401。
必须解析全部 cookie、按名去重、并让 `session` 排在请求头最前。
另外 Loon 可能把多个 Set-Cookie 合并成一串（逗号或换行分隔），需要拆开。

## 日志核验的坑

查 `/api/log/self` **必须带 `type=4`**：

```
GET /api/log/self?p=1&page_size=20&type=4
```

原因：日志里绝大多数是 API 调用记录（type=2/5），签到记录（type=4）会被挤出首页。
账号日志量大时（实测 440 条），`page_size=500` 也会被服务端截到 100，**前 100 条里一条签到都没有**。

另外 `type=4` 里还混着「新用户注册赠送 ＄100」这类记录，所以判定**优先匹配内容含「签到成功」**，
匹配不到才退回按 `type=4` 取最新，避免把注册赠送误判成签到。
