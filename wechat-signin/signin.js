...
// 默认值仅在持久存储为空时使用
const DEFAULT_SESSION = "763df6f3-9ee2-414b-8a35-c5774b041273";
const DEFAULT_USERID = "1819640511414149121";
// 签到动作接口（已从完整抓包确认）
...
function buildHeaders(session) {
    return {
        ...
        "Referer": "https://servicewechat.com/" + APP_ID + "/39/page-frame.html"
    };
}
...
(function () {
    const session = $persistentStore.read(KEY_SESSION) || DEFAULT_SESSION;
    const userId = $persistentStore.read(KEY_USERID) || DEFAULT_USERID;
    const capTime = $persistentStore.read(KEY_TIME);
    console.log("session: " + session.substring(0, 8) + "... userId: " + userId +
        (capTime ? " (捕获于 " + capTime + ")" : " (使用默认硬编码值，建议先打开小程序刷新)"));
    ...
    req({
        ...
        if (info.code !== 0) {
            notify("⚠️ 凭证已失效", "code: " + info.code, "请打开微信小程序任意页面刷新 session 后重试");
            return finish();
        }
